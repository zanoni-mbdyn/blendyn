...
def update_defjoint(elem, insert_keyframe = False):
    ...
    axes = ['X', 'Y', 'Z']
    for ii in range(3):
        defjointChild = bpy.data.objects[defjointOBJ.name + '.' + axes[ii]]

        #Apply Linear Deformation
        defjointChild.location = Vector(p1) + Vector(def_vector[:ii] + (0, )* (3 - ii))
        defjointChild.dimensions[2] = def_vector[ii] 

        #Apply Angular Deformation
        defjointChild.modifiers['SimpleDeform'].angle = phi[ii]
...
def spawn_defjointj_element(elem, context):
    ...
    for ii in range(3):
        ...
        obj_loc = (p2 - p1)
        defjointChild.location = Vector(p1) + Vector(obj_loc[:ii] + (0, )* (3 - ii))
        ...
        defjointChild.name = defjointOBJ.name + '.' + axes[ii]

        bpy.context.scene.objects.active = defjointChild
        bpy.ops.object.modifier_add(type='SIMPLE_DEFORM')
        defjointChild.modifiers['SimpleDeform'].deform_method = 'TWIST'
