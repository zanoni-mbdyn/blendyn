...
def parse_axialrot(rw, ed):
    ret_val = True
    # Debug message
    print("parse_axialrot(): Parsing axialrot joint " + rw[1])
    try:
        el = ed['axialrot_' + str(rw[1])]

        print("parse_axialrot(): found existing entry in elements dictionary. Updating it.")
        ...
        print("parse_axialrot(): didn't found en entry in elements dictionary. Creating one.")
        el = ed.add()
        el.type = 'axialrot'
...
def spawn_axialrot_element(elem, context):
    ...
    if any(obj == elem.blender_object for obj in context.scene.objects.keys()):
        ...
        print("spawn_axialrot_element(): Element is already imported. \
    ...
    try:
        ...
        print("spawn_axialrot_element(): Could not find a Blender \
    ...
    if app_retval == {'FINISHED'}:
        ...
        axialrotjOBJ.rotation_quaternion = \
